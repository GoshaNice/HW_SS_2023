...
class GlobalLayerNorm(nn.Module):
    """
    Global Layer Normalization (gLN in the article).
    """
...
class TCNBlock_extractor(nn.Module):
    def __init__(self, in_channels, out_channels, is_first = False, spk_embed_dim = 1, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.block = nn.Sequential(
            nn.Conv1d(in_channels=in_channels + spk_embed_dim * is_first, out_channels=out_channels, kernel_size=1),
            nn.PReLU(),
...
class SpExPlus(nn.Module):
    def __init__(self, 
        ...
        self.encoder_middle = nn.Sequential(
            nn.Conv1d(in_channels=1, out_channels=N, kernel_size=L2, stride=L1//2, padding=(L2 - L1)//2),
            nn.ReLU()
        )
        self.encoder_long = nn.Sequential(
            nn.Conv1d(in_channels=1, out_channels=N, kernel_size=L3, stride=L1//2, padding=(L3 - L1)//2),
            nn.ReLU()
        )
        ...
        self.mask1 = nn.Sequential(
            nn.Conv1d(in_channels=proj_dim, out_channels=N, kernel_size=1),
            nn.ReLU()
        )
        ...
        self.decoder_short = nn.ConvTranspose1d(N, 1, kernel_size=L1, stride=L1 // 2, bias=True)
        self.decoder_middle = nn.ConvTranspose1d(N, 1, kernel_size=L2, stride=L1 // 2, bias=True)
        self.decoder_long = nn.ConvTranspose1d(N, 1, kernel_size=L3, stride=L1 // 2, bias=True)
